...
function submitForm(){
	...
	ajax.open( "POST", "update_appointment_db.php" );
...
<div class="row">
<h2 class="h2" style="text-align:center;">Update Appointment: <input type="text" id="app_num" style="border:none;color:#0000CC;" readonly></h2>
</div><br>
...
<div class="row">
 <?php //Include database configuration file
    ...
    $query = mysqli_query($con,"SELECT * FROM app_yms ORDER BY APP_NUM DESC");
...
<input type="text" class="form-control" id="app_title" placeholder="DockDoor" required>
</div>

<div class="col-md-3">

 
<input type="text" name="yard" class="form-control" id="yard" placeholder="Yard"  required>
...
<input type="text" name="equip_name" class="form-control"  id="equip_name" placeholder="Yard" required>
...
<input type="submit" id="btnsub" class="btn btn-primary active" value="Update Appointment" >
...
$(document).ready(function(){
 ...
 var ob = JSON.parse(response);
     ...
     $("#app_title").val(ob.DOCK_DOOR);
     $("#yard").val(ob.YARD	);
     $("#equip_name").val(ob.EQUIP_NAME);  
     $("#datepicker").val(ob.DATE); 
	   $("#start").val(ob.START); 
	   $("#end").val(ob.END); 
     $("#driver_name").val(ob.DRIVER_NAME); 
	 $("#app_status").val(ob.APP_STATUS);  
	 $("#doc_type").val(ob.DOC_TYPE); 
	  $("#doc_num").val(ob.DOC_NUM); 
	   $("#dock").val(ob.DOCK);  
     $("#app_car").val(ob.APP_CAR);  
     $("#app_ship").val(ob.APP_SHIP);  
